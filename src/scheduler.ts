...
cron.schedule('*/10 * * * *', async () => {
  ...
  for (const lead of leads || []) {
    ...
    try {
      ...
      await supa.from('call_attempts').insert({ lead_id: lead.id, attempt_no: lastNo+1, started_at: new Date().toISOString(), retell_call_id: resp.call_id, outcome: 'initiated' });
      await supa.from('leads').update({ status: 'calling', next_retry_at: null }).eq('id', lead.id);
    } catch(e:any){ log.error('retry dial error', e.message); }
  }
});

cron.schedule('5 * * * *', async () => {
  const { data: leads, error } = await supa.from('leads').select('*').eq('status','whatsapp_outreach');
  if (error) return;
  for (const lead of leads || []) {
    const to = lead.whatsapp || (lead.phone?.startsWith('whatsapp:') ? lead.phone : `whatsapp:${lead.phone}`);
    if (!to) continue;
    try {
      await twilio.messages.create({
        to, messagingServiceSid: process.env.TWILIO_MESSAGING_SERVICE_SID,
        body: `Olá ${lead.name.split(' ')[0]}! Tentamos falar por telefone. Você prefere continuar por *ligação* ou *WhatsApp*? Responda "ligar" ou "WhatsApp".`
      });
      await supa.from('leads').update({ status:'waiting_preference' }).eq('id', lead.id);
    } catch {}
  }
});